function visualize_strands_via_color_V200( strand_subscripts, vessel_directions, max_strand_energies, ...
                                           ...
                                           ~, is_inverted_original, are_vectors_opaque, color_code, microns_per_voxel, varargin )
%% SAM 12/12/17 
...
if ~ isempty( varargin ), strand_r = varargin{ 1 }; dimensionality = size( strand_r{ 1 }, 2 ); end
...
if ~ strcmp( color_code, 'xy-directions' ) ...
&& ~ strcmp( color_code,  'r-directions' ),      strand_r = strand_subscripts ; end

size_of_crop_backup             =            size_of_crop ;
original_image_backup           =          original_image ;
strand_subscripts_backup        =       strand_subscripts ;
vessel_directions_backup        =       vessel_directions ; % don't rotate
strand_r_backup                 =                strand_r ; % don't rotate
pixels_per_sigma_range_backup   =  pixels_per_sigma_range ;
y_limits_backup                 =                y_limits ;
x_limits_backup                 =                x_limits ;
z_limits_backup                 =                z_limits ;
microns_per_voxel_backup        =       microns_per_voxel ;
max_strand_energies_backup      =     max_strand_energies ;

ortho_view_ordinate_range = 0 : 2 ; 
ortho_view_ordinate_range = mod( ortho_view_ordinate_range, 3 ); 
%  dim order|ortho view index|ortho projection
% [ Y, X, Z ]       0               x,y
% [ X, Z, Y ]       1               z,x
% [ Z, Y, X ]       2               y,z
ortho_view_names{ 1 }   = 'Z' ;
ortho_view_names{ 2 }   = 'Y' ;
ortho_view_names{ 3 }   = 'X' ;

% for ortho_index = 1 : 3
for ortho_view_ordinate = ortho_view_ordinate_range

                                    [     size_of_crop       , original_image       , strand_subscripts       , pixels_per_sigma_range       , y_limits       , x_limits       , z_limits       , microns_per_voxel ] ...
  = rotate_geometry( ortho_view_ordinate, size_of_crop_backup, original_image_backup, strand_subscripts_backup, pixels_per_sigma_range_backup, y_limits_backup, x_limits_backup, z_limits_backup, microns_per_voxel_backup );
    
    vessel_directions        =       vessel_directions_backup ; % don't rotate
    strand_r                 =                strand_r_backup ; % don't rotate
    max_strand_energies      =     max_strand_energies_backup ;

    original_MIP = double( max( original_image, [ ], 3 ));
    
    % histogram( original_image_max_intensity_projected( : ))
    
    intensity_limits = quantile( original_MIP( : ), [ 0.01, 0.99 ]);
    ...
    strand_r         =         strand_r( logical_strands_in_crop );
...
%     strand_r         =         strand_r( indices_sorted_by_size );    
    ...
    switch color_code
        case 'strands'
            rng('default') % do this for consistency in random strand color between the 3 ortho views and also the 3D visualization FXN    
            strand_colors = 0.1 + 0.8 * rand( number_of_strands, 3 );
    end
    ...
    %     radii_in_pixels_at_strand = radii_in_pixels_range( subscripts_at_strand( :, 4 ), : );
        ...
        directions_at_strand = vessel_directions{ strand_index }( within_bounds_strands_logical, : );
          strand_r_at_strand =          strand_r{ strand_index }( within_bounds_strands_logical, : );
        
        switch color_code
    
            case 'strands'
                
                colors_at_strand = strand_colors( strand_index, : ) .* ones( size( subscripts_at_strand, 1 ), 1 );            
...
%                 colors_at_strand( 1, : ) = sum( colors_at_strand([ 1, 3 ], : ) .^ 2, 1 ) .^ 0.5 ;
%     
% %                 colors_at_strand( 3, : ) = zeros( size( subscripts_at_strand( :, 1 )' ));
%                 colors_at_strand( 3, : ) = colors_at_strand( 1, : );
                colors_at_strand( :, 1 ) = sum( colors_at_strand( :, [ 1, 2 ]) .^ 2, 2 ) .^ 0.5 ;
    
%                 colors_at_strand( 3, : ) = zeros( size( subscripts_at_strand( :, 1 )' ));
%                 colors_at_strand( :, 2 ) = 0.5 * ones( size( subscripts_at_strand( :, 1 ) ));
                colors_at_strand( :, 2 ) ...
              = colors_at_strand( :, 1 );


            case { 'xy-directions', 'r-directions' }
% 
                if dimensionality == 2, dims = [ 2, 1    ];
                else,                   dims = [ 2, 1, 3 ];
                end

%                 % sqt(y2 + x2), 1, proj( r, sqt(y2 + x2))  -> R, G, B
                strand_r_component = abs( dot( directions_at_strand( :, dims ), strand_r_at_strand( :, dims ), 2 )) ...
                                  ./      dot(   strand_r_at_strand( :, dims ), strand_r_at_strand( :, dims ), 2 ) .^ 0.5 ;

                colors_at_strand = [ 0,  0 , 1 ] .*       strand_r_component ...
                                 + [ 0, 1/2, 0 ] ...
                                 + [ 1,  0 , 0 ] .* ( 1 - strand_r_component );

            case 'radius'

%                 strand_radius_normalized = (  positions_at_strand( :, 4 )     - min( strand_subscripts_mat )) ...
%                                          / ( max( strand_subscripts_mat ) - min( strand_subscripts_mat ));
                strand_radius_normalized = (       subscripts_at_strand( :, 4 )   - min( strand_subscripts_mat( :, 4 ) )) ...
                                         / ( max( strand_subscripts_mat( :, 4 ) ) - min( strand_subscripts_mat( :, 4 ) ));

                colors_at_strand = [ 1,  0 , 0 ] .*       strand_radius_normalized ...
                                 + [ 0, 1/2, 0 ]                                  ...
                                 + [ 0,  0 , 1 ] .* ( 1 - strand_radius_normalized );

                
    
        end

        colors_at_strand = colors_at_strand' ;
...
%         gcf().Children(2).Children %%%%%%%%%%%%%% lights lighting !!!!! change angle by 180 degrees about z axis for Dwight !!!! %%%%%%%%%%
    ...
    switch color_code
        ...
        case        'radius' , name =       'Radius' ;
        case       'strands' , name =      'Strands' ;
        case    'directions' , name =    'Direction' ;
        case  'z-directions' , name =  'Z-Direction' ;
        case  'r-directions' , name =  'R-Direction' ;
        case 'xy-directions' , name = 'XY-Direction' ;
    
    end
    
    set( h, 'Name', [ network_handle, '_', original_handle( 10 : end ), ' ', name, ' ', ortho_view_names{ ortho_view_ordinate + 1 }, '-Projection' ]) % first 9 characters: original_
    
    image( composite_image )
    
    % tick_spacing = 50 ; % microns
    tick_spacing = 200 ; % microns
    
    dims =  ceil( y_limits( 1 ) * microns_per_voxel( 1 ) / tick_spacing ) ...
         : floor( y_limits( 2 ) * microns_per_voxel( 1 ) / tick_spacing );...
         
    y_Tick_labels = dims * tick_spacing ;
    ...
    dims =  ceil( x_limits( 1 ) * microns_per_voxel( 2 ) / tick_spacing ) ...
         : floor( x_limits( 2 ) * microns_per_voxel( 2 ) / tick_spacing );...
         
    x_Tick_labels = dims * tick_spacing ;
...
%             map( 1 : 64, 1 : 3 ) = linspace( 0, 1, 64 )';
            map( 1 : 64, 1 ) = linspace( 1, 0, 64 ) .^ 2';
            map( 1 : 64, 3 ) = linspace( 0, 1, 64 ) .^ 2';
            map = map .^ 0.5 ;

            map = map * ( 1 - weight ) + weight ;
...
function                    [ size_of_crop, original_image, strand_subscripts, pixels_per_sigma_range, y_limits, x_limits, z_limits, microns_per_voxel ] ...
           = rotate_geometry( direction, ...
                              size_of_crop, original_image, strand_subscripts, pixels_per_sigma_range, y_limits, x_limits, z_limits, microns_per_voxel )
%     if ~ isempty( varargin{ 1 }), strand_r = varargin{ 1 }; end ... vessel_directions, 

    dim_order = mod(( 0 : 2 ) + direction, 3 ) + 1 ;
%     dim_order = [ 3, 1, 2 ], for direction == +1

    size_of_crop           =           size_of_crop(      dim_order     );
    microns_per_voxel      =      microns_per_voxel(      dim_order     );

    pixels_per_sigma_range = pixels_per_sigma_range( :,   dim_order     );

%     vessel_directions      = cellfun( @( x )      x( :,   dim_order     ), vessel_directions, 'UniformOutput', false ); % !!!!! keep the VESSEL directions constant to preserve color-coding across views
%     strand_r               = cellfun( @( x )      x( :,   dim_order     ),          strand_r, 'UniformOutput', false ); % !!!!! keep the RADIAL directions constant to preserve color-coding across views
    strand_subscripts      = cellfun( @( x )      x( :, [ dim_order, 4 ]), strand_subscripts, 'UniformOutput', false );
    
    for direction_idx = 1 : abs( direction ) % permute variable names (rotate the ring of integers mod 3)
    ...
    original_image = permute( original_image, dim_order );    

end % FUNCTION rotate_geometry()
