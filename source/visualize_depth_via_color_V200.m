function visualize_depth_via_color_V200( edge_subscripts, max_edge_energies, pixels_per_sigma_range,     ...
                                         sigma_per_size, data_directory, original_handle,                   ...
                                         y_limits, x_limits, z_limits, contrast_limit, intensity_limits, is_inverted_original )
%% SAM 12/12/17 
...
% 5/28/18
%
% V4 which accepts non integer edge subscripts.  SAM 7/16/18
%
% V5, fixed the cropping bug that changed the size of the edges_subscripts array during cropping
...
% round the outlying MIP intensities
...
% % scale the max_edge_energies by the input contrast limits
% max_edge_energies                                      ...
%     = 255                                              ...
%       - (   max_edge_energies  - contrast_limit( 1 )) ...
%       / ( contrast_limit( 2 ) - contrast_limit( 1 )) ...
%       * 255 ;
...
for scale_index = 1 : number_of_scales
    ...
    % NOTE:  this prevoius line does not do its supposed duty.  should do unique by 'rows' and only
    % on x and y coordinates (not z)
...
if ~ is_inverted_original, inverted_original_MIP( logical( max( centerlines_image, [ ], 3 ))) = 0 ; end % make vectors opaque

composite_image = uint8( centerlines_image + double( inverted_original_MIP ));
