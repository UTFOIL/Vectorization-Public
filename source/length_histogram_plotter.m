...
%% sorting, binning
[ x1, dx1, dy1 ] = sort_and_bin_by_statistic( inner_pos_z,            delta_lengths, number_of_bins,  z_pos_limits );
[ x2, dx2, dy2 ] = sort_and_bin_by_statistic( inner_pos_radii,        delta_lengths, number_of_bins, radius_limits );
[ x3, dx3, dy3 ] = sort_and_bin_by_statistic( inner_pos_inclinations, delta_lengths, number_of_bins, inclin_limits );

 x = [  x1;  x2;  x3 ];
dx = [ dx1; dx2; dx3 ];
dy = [ dy1; dy2; dy3 ];

function [ x, dx, dy ] = sort_and_bin_by_statistic( inner_statistics, delta_lengths, number_of_bins, statistic_limits )
...
%     bin_limits = linspace( min( inner_statistics ), max( inner_statistics ), number_of_bins + 1 );
    ...
    % summing lengths
    bin_length_sums = cellfun( @sum, delta_lengths_bin_cell );   
...
for stat_i = 1 : 3 
    ...
    if stat_i == 1, ylabel( 'length [mm]' ); end
