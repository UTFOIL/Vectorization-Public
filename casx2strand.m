function [ vertex_space_subscripts, vertex_scale_subscripts, bifurcation_vertices, strands2vertices, strand_space_subscripts, strand_scale_subscripts, microns_per_voxel, lumen_radius_in_microns_range, size_of_image ] = casx2strand( point_coordinates, arc_connectivity, arc_diameters )
...
% switch x and y and rotate z to point downward

point_coordinates( :, [ 2, 3 ]) = - point_coordinates( :, [ 2, 3 ]);

point_coordinates = point_coordinates( :, [ 2, 1, 3 ]);

% force the arc_connectivity table to be upper triangular (first index lower than second index)
is_arc_switching = arc_connectivity( :, 2 ) < arc_connectivity( :, 1 );

% every row where the vertex index on the right column is larger requires a permutation of columns
arc_connectivity = [ arc_connectivity( :, 1 ), arc_connectivity( :, 2 )] .* ~ is_arc_switching ...
                 + [ arc_connectivity( :, 2 ), arc_connectivity( :, 1 )] .*   is_arc_switching ;
...
points2radii          = zeros( max_point_index, 1     );
points2number_of_arcs = zeros( max_point_index, 1     );
% points2radii_i        =                 point_indices  ;
...
%     index_i = index_i + 1 ;
    ...
    points2number_of_arcs( point_index ) = numel( point2arc_or_arcs );
...
microns_per_voxel = [ 1, 1, 1 ]; % microns and voxels are interchangeable

scales_per_octave = 7 / 2 ;

radii_min = full( min( nonzeros( points2radii ))) / 2 ^ ( 1 / scales_per_octave ); % subtract a scale
radii_max = full( max(           points2radii  )) * 2 ^ ( 1 / scales_per_octave ); % add a scale to ensure interpolation will be in bounds
...
is_point_used = points2radii > 0 ;

min_object_coordinates = min( point_coordinates( point_indices( is_point_used )' ) - nonzeros( points2radii ));
max_object_coordinates = max( point_coordinates( point_indices( is_point_used )' ) + nonzeros( points2radii ));
...
size_of_image = ceil( max_object_coordinates - min_object_coordinates + 1 );
...
% adjacency_matrix           = ones( size( arc_connectivity( :, 1 )), 'logical' );
adjacency_matrix         =             arc_connectivity( :, 1 ) ...
                                   + ( arc_connectivity( :, 2 ) - 1 ) * max_point_index ;
                     
% points2number_of_arcs = sum( adjacency_matrix, 1 ) + sum( adjacency_matrix, 2 )';
...
is_point_a_vertex = points2number_of_arcs > 2 | points2number_of_arcs == 1 ;
...
is_vertex_a_bifurcation = points2number_of_arcs( is_point_a_vertex ) > 2 ;
...
number_of_repeated_points = sum( points2number_of_arcs( vertices2points ) - 1 );

number_of_strands = sum( points2number_of_arcs( vertices2points )) / 2 ;
...
%     point_B_indices = find( adjacency_matrix( :, point_A_index )' + adjacency_matrix( point_A_index, : ));
    ...
    adjacency_matrix_j = ceil( adjacency_matrix / max_point_index );

    is_A_at_j = adjacency_matrix_j == point_A_index ;

    point_B_indices1 = mod( adjacency_matrix( is_A_at_j ) - 1, max_point_index ) + 1 ;

    adjacency_matrix_i = mod( adjacency_matrix - 1, max_point_index ) + 1 ;

    is_A_at_i = adjacency_matrix_i == point_A_index ;
    
    point_B_indices2 = ceil( adjacency_matrix( is_A_at_i ) / max_point_index );
    
    point_B_indices = [ point_B_indices1; point_B_indices2 ]' ;
    
    is_A_at_point = is_A_at_i | is_A_at_j ;
...
%         if adjacency_matrix( point_B_index, point_A_index ) || adjacency_matrix( point_A_index, point_B_index )
        if any( adjacency_matrix == min( point_A_index, point_B_index ) + ( max( point_A_index, point_B_index ) - 1 ) * max_point_index )
...
%                     point_C_index_new = find( adjacency_matrix( :, point_C_index )' + adjacency_matrix( point_C_index, : ));                    

                    adjacency_matrix_j = ceil( adjacency_matrix / max_point_index );

                    is_C_at_j = adjacency_matrix_j == point_C_index ;
                    
                    point_C_index_new1 = mod( adjacency_matrix( is_C_at_j ) - 1, max_point_index ) + 1 ;

                    adjacency_matrix_i = mod( adjacency_matrix - 1, max_point_index ) + 1 ;

                    is_C_at_i = adjacency_matrix_i == point_C_index ;
                    
                    point_C_index_new2 = ceil( adjacency_matrix( is_C_at_i ) / max_point_index );

                    point_C_index_new = [ point_C_index_new1; point_C_index_new2 ] ;
                    
                    if point_C_index == point_B_index
                        % then there will be two neighbors
                        
                        % we don't want to find the point_A index and go back where we started
                        point_C_index_new( point_C_index_new == point_A_index ) = [ ];
...
%                     adjacency_matrix( point_C_index, : ) = 0 ;
                    ...
                    is_C_at_point = is_C_at_i | is_C_at_j ;

                    adjacency_matrix( is_C_at_point ) = [ ];
                    
                    is_A_at_point( is_C_at_point ) = [ ];

                    point_C_index = point_C_index_new ;
...
%     adjacency_matrix( point_A_index, : ) = 0 ;
    adjacency_matrix( is_A_at_point ) = [ ];
