...
% curvatures_gaussian_kernel_dft = curvatures_kernels_dft .* reshape( blurred_matching_kernel_dft, [ 1, size_of_chunk_dft ]);
%   gradient_gaussian_kernel_dft =   gradient_kernels_dft .* reshape( blurred_matching_kernel_dft, [ 1, size_of_chunk_dft ]);

Laplacian_matching_kernel_dft = Laplacian_kernel_dft .* matching_kernel_dft ;

%   gradient_gaussian_kernel_dft =   gradient_kernels_dft .* curvatures_gaussian_kernel_dft( 1 : 3, :, :, : );

% % normalize each derivative kernel
...
Laplaican_chunk_dft = Laplacian_matching_kernel_dft .* chunk_dft ;

Laplaican_chunk = ifftn( Laplaican_chunk_dft, 'symmetric' );
...
% % for curvature_index = 1 : 6
% for curvature_index = 1 : 3
% 
...
% Laplaican_chunk = sum( curvatures_chunk(( 1 : 3 )' + 6 * ( voxel_index_full_range - 1 )), 1 );

energy_chunk = Laplaican_chunk( local_ranges{ 1 }, ...
                                local_ranges{ 2 }, ...
                                local_ranges{ 3 }  );

% laplacian_all_the_way = true ;
% 
% if laplacian_all_the_way
%     
%     energy_chunk = reshape( Laplaican_chunk, size_of_chunk );
%     
% else
%     
%     valid_voxels = Laplaican_chunk < 0 ;
% 
