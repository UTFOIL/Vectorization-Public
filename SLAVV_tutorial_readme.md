...
## Visualizations
Use the ```network_*.vmv``` file output in the ```vectors``` directory of the ```batch_*``` folder to export the vascular vectorization to the [VessMorphoVis](https://github.com/BlueBrain/VessMorphoVis) plugin to Blender to visualize the network in 3D with lighting and color-coding.  
